"""
  Sudoku Solver V2 which takes less time to solve
"""
...
def solve_sudoku(grid):
    ...
    for i in get_possible_values(grid, row, col):
...
def get_possible_values(grid, row_index, col_index):
    ...
    possible_values = list(set(range(10)) - set(row_data + column_data + grid_data))

    try:
      possible_values.remove(0)
    except ValueError:
        pass
#    print "Possible values at (%s,%s): %s" % (row_index, col_index, possible_values)
